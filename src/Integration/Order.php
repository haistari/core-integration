		...
		else
		{
			try  
			{
														...
														if(!$redis->exists("orderheader:".$order['order_code'].":".$client['client_id']))
														{
															...
															if($stmt_checkOrder->rowCount() == 0)
															{
																...
																$data_order = array(
																	...
																	":order_source"			 => (isset($order['order_source'])) ? $order['order_source'] : "SCHEDULER",
																	":shop_name"             => $order['shop_name'],
				...
				else
				{
					...
					$itemList = trim($itemList, "]");
					$itemList = str_replace('"', '', strval($itemList));
